...
class Commandes {
    ...
    function recupererCom($reference){
        $sql="SELECT * from commande where type_client='$reference' "; //( type_client === id_client)
    ...
    function recupererQUantite_Produit($reference){
...
class Paniers {
    ...
    function supprimerPaniers_apres_commande($reference){
        $sql="DELETE FROM panier where ID_Client= :reference";
        $db = config2::getConnexion();
        $req=$db->prepare($sql);
        $req->bindValue(':reference',$reference);
        try{
...
*/
    ...
    function Refresh_Quantite_Base($reference,$quantite){
        $Commandes = new Commandes();
        $res = $Commandes->recupererQUantite_Produit($reference);
        foreach($res as $key){$old_q=$key['quantite'];}
        $new_qq = $old_q - $quantite ;

        $sql="UPDATE produit SET quantite='$new_qq' where reference='$reference' ";
