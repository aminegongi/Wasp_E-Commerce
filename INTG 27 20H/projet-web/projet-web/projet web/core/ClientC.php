...
class ClientC {
	function ajouterClient($Client){
		$sql="insert into Client (ID,pseudo,mail,Adresse,image,passwd,NumTel,nom,prenom,date) values (:ID,:pseudo,:mail,:Adresse,:image,:passwd,:NumTel,:nom,:prenom,:date)";
	...
	function modifierClient($Client,$ID){
		$sql="UPDaTE client SET pseudo=:pseudo , mail=:mail , Adresse=:Adresse , image=:image , passwd=:passwd , NumTel=:NumTel , nom=:nom , prenom=:prenom where ID=:ID";
		...
		try{		
			...
			$datas = array(':ID'=>$ID, ':pseudo'=>$Pseudo, ':mail'=>$Mail,':Adresse'=>$Adresse,':image'=>$Image,':passwd'=>$Passwd,':nom'=>$Nom,':prenom'=>$Prenom);
			$req->bindValue(':ID',$ID);
			$req->bindValue(':pseudo',$Pseudo);
			$req->bindValue(':mail',$Mail);
