                    ...
                    <div class="row">
                        ...
                        <div class="col-lg-6">
                            <div class="login">
                                ...
                                <h3 style="color:black;"><?php echo $_SESSION['recup_mail']; ?> Récuperation du Mot de Passe</h3>
                                <div class="form_login">
                                    <form method="POST" action="ForgotB.php" >
                                        <div class="form-group">
                                        <label>Votre Code :</label>
                                        <input type="text" required="required" class="form-control" placeholder="Code de vérification" name="verif_code"/><br/>
                                        </div>
                                        <button type="submit" name="verif_submit" value="Valider" class="btn login_button trans_200">Envoyer !</button>
                                        <?php if(isset($error)) { echo '<span style="color:red">'.$error.'</span>'; } else { echo ""; } ?>
                                    </form>
                                </div>
                                <?php } ?>
                                <?php if($_GET['section'] == "changemdp"){ ?>
                                <h3 style="color:black;"><?php echo $_SESSION['recup_mail']; ?> Récuperation du Mot de Passe</h3>
                                <div class="form_login">
                                    <form method="POST" action="ForgotB.php" >
                                        <div class="form-group">
                                        <input type="password" required="required" class="form-control" placeholder="Nouveau mot de passe" name="change_mdp"/><br/>
                                        <input type="password" required="required" class="form-control" placeholder="Confirmation du mot de passe" name="change_mdpc"/><br/>
                                        </div>
                                        <button type="submit" value="Valider" name="change_submit" class="btn login_button trans_200">Envoyer !</button>
                                        <?php if(isset($error)) { echo '<span style="color:red">'.$error.'</span>'; } else { echo ""; } ?>
                                    </form>
                                </div>
                                <?php } ?>
